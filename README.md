# Caderno Temático: Renda Fixa para Iniciantes

## Contexto e Objetivos

Este projeto foi desenvolvido com o objetivo de utilizar a Inteligência Artificial como ferramenta de aprendizagem ativa para aprofundar meus conhecimentos sobre renda fixa.

O tema escolhido foi "Renda Fixa para Iniciantes", pois esse tipo de investimento representa uma das principais portas de entrada para pessoas que desejam iniciar no mercado financeiro e eu tenho interesse nessa parte.

Os objetivos deste estudo são:

- Compreender o conceito de renda fixa;
- Conhecer os principais tipos de investimentos dessa categoria;
- Entender conceitos como rentabilidade, liquidez e risco;
- Criar um material de revisão utilizando o NotebookLM.

## Curadoria de Fontes

As fontes utilizadas para construção deste caderno:

1. Genial Investimentos - Guia Definitivo da Renda Fixa (PDF)
Link: https://media-blog.genialinvestimentos.com.br/wp-content/uploads/2023/07/24175437/genial-investimentos-o-guia-definitivo-renda-fixa-20230721.pdf

2. Banco do Brasil Investalk - Renda Fixa: tudo o que você precisa saber para investir com segurança
Link: https://investalk.bb.com.br/noticias/quero-aprender/renda-fixa

3. InfoMoney - Renda Fixa: tudo o que você precisa saber para começar a investir
Link: https://www.infomoney.com.br/guias/renda-fixa/

4. B3 Bora Investir - Renda Fixa - O que é, significado e definição
Link: https://borainvestir.b3.com.br/glossario/renda-fixa/

5. Banco Central do Brasil - Glossário Simplificado de Termos Financeiros
Link: https://www.bcb.gov.br/content/cidadaniafinanceira/documentos_cidadania/Informacoes_gerais/glossario_cidadania_financeira.pdf

6. Tesouro Direto
Link: https://www.tesourodireto.com.br/

# Engenharia de Prompts e Cicatrizes

## Prompt 1 - Conceito de Renda Fixa

### Pergunta realizada no NotebookLM:

'Explique o que é renda fixa para uma pessoa que nunca investiu. Utilize as fontes fornecidas e apresente exemplos práticos.'

### Resultado obtido resumo:

A resposta explicou que a renda fixa funciona como um empréstimo realizado pelo investidor para um emissor (governo, banco ou empresa), que devolve o valor aplicado com uma remuneração definida.

Foram apresentados exemplos de investimentos:

- Tesouro Direto: investimento em títulos públicos emitidos pelo Governo Federal;
- CDB: investimento em títulos emitidos por bancos;
- LCI e LCA: títulos relacionados aos setores imobiliário e agrícola.

Também foram explicadas as formas de rentabilidade:

- Prefixada: o investidor conhece a taxa de retorno no momento da aplicação;
- Pós-fixada: acompanha indicadores como CDI ou Selic;
- Híbrida: combina uma taxa fixa com um índice como o IPCA.

### Dificuldade encontrada:

A primeira resposta estava correta, porém poderia ser melhor organizada para um estudante iniciante, ficou muito resumida e muitos assuntos.

### Melhoria realizada:

Foi criado um novo prompt solicitando uma estrutura dividida em definição, funcionamento, exemplos, vantagens, riscos e cuidados.

---

## Prompt 2 - Organização do conteúdo para estudo

### Pergunta realizada:

"Explique renda fixa para um estudante iniciante de tecnologia, organizando a resposta em:
- definição;
- funcionamento;
- exemplos;
- vantagens;
- riscos;
- cuidados."

### Resultado obtido resumo:

### Definição
Renda fixa é uma modalidade de investimento onde as regras de remuneração são conhecidas previamente ou seguem indicadores econômicos.

### Funcionamento
O investidor empresta dinheiro para um emissor e recebe o valor aplicado acrescido de juros.

### Exemplos
- Tesouro Direto;
- CDB;
- LCI e LCA;
- Debêntures.

### Vantagens
- Maior previsibilidade;
- Possibilidade de proteção contra inflação;
- Segurança em determinados investimentos.

### Riscos
- Risco de crédito;
- Risco de mercado;
- Risco de liquidez.

### Cuidados
- Avaliar o emissor;
- Observar prazo e liquidez;
- Entender a tributação.

### Melhoria realizada:

Ficou melhor organizado em tópicos facilitando o entendimento para um iniciante.

## Prompt 3 - Comparação dos investimentos de renda fixa

### Pergunta realizada:

Compare os principais investimentos de renda fixa (Tesouro Direto, CDB, LCI, LCA e Debêntures) utilizando as fontes fornecidas.
Organize a resposta em uma tabela contendo:
-O que é;
-Quem emite;
-Como funciona a rentabilidade;
-Nível de risco;
-Possui garantia do FGC ou não;
-Indicação para qual perfil de investidor.

### Resultado obtido resumo:

O NotebookLM apresentou uma comparação entre os investimentos em formato de tabela bem completo.

### Dificuldade encontrada:

Inicialmente, comparar os investimentos de renda fixa apenas pela rentabilidade poderia gerar uma visão incompleta. Foi necessário considerar também fatores como risco, liquidez, emissor e garantias.

### Aprendizado:

A comparação mostrou que investimentos de renda fixa possuem características diferentes. Apesar de todos terem regras de remuneração definidas, existem diferenças importantes relacionadas ao emissor, risco, liquidez e garantias e objetivos de cada investidor.


## Miniguia de Estudo

### O que é Renda Fixa?

Renda fixa é uma modalidade de investimento em que o investidor empresta dinheiro para um emissor, como o governo, um banco ou uma empresa, recebendo em troca uma remuneração previamente definida ou vinculada a um indicador econômico. Em comparação com a renda variável, oferece maior previsibilidade dos rendimentos e costuma ser indicada para investidores que buscam mais segurança.

---

### Como funciona?

Ao investir em um título de renda fixa, o investidor financia o emissor por um determinado período. Em troca, recebe o valor investido acrescido de juros no vencimento ou conforme as regras do investimento.

A rentabilidade pode ser:

- **Prefixada:** taxa definida no momento da aplicação.
- **Pós-fixada:** acompanha um indicador, como CDI ou Selic.
- **Híbrida:** combina uma taxa fixa com um índice de inflação, como o IPCA.

---

### Principais investimentos

### Tesouro Direto

- Emitido pelo Governo Federal.
- Considerado um dos investimentos mais seguros do país.
- Indicado para reserva de emergência e objetivos a longo prazo.

### CDB

- Emitido por bancos.
- Pode possuir rentabilidade prefixada, pós-fixada ou híbrida.
- Conta com proteção do FGC dentro dos limites estabelecidos.

### LCI e LCA

- Emitidas por instituições financeiras.
- Destinadas ao financiamento dos setores imobiliário e do agronegócio.
- Possuem isenção de Imposto de Renda para pessoas físicas, conforme a legislação vigente.

### Debêntures

- Emitidas por empresas.
- Geralmente oferecem maior potencial de retorno, porém apresentam maior risco.
- Não possuem cobertura do FGC.

---

### Vantagens

- Maior previsibilidade dos rendimentos.
- Diversas opções de investimento.
- Possibilidade de proteção contra a inflação.
- Alternativas com alta segurança.

---

###b Riscos

- Risco de crédito.
- Risco de mercado.
- Risco de liquidez.

## Glossário

**CDI**  
Taxa utilizada como referência para diversos investimentos de renda fixa.

**Selic**  
Taxa básica de juros da economia brasileira.

**IPCA**  
Índice oficial utilizado para medir a inflação no Brasil.

**FGC (Fundo Garantidor de Créditos)**  
Instituição que protege investidores em determinados produtos financeiros dentro dos limites previstos na regulamentação.

**Liquidez**  
Facilidade de transformar um investimento em dinheiro.

**Rentabilidade**  
Retorno obtido por um investimento.

**Prefixado**  
Título cuja taxa de rendimento é conhecida no momento da aplicação.

**Pós-fixado**  
Título cuja rentabilidade acompanha um indicador econômico.

**Híbrido**  
Título que combina uma taxa fixa com um índice de inflação.

**Tesouro Direto**  
Programa de compra de títulos públicos emitidos pelo Governo Federal.

## Prompts Reutilizáveis

### Prompt 1

Explique o que é renda fixa para uma pessoa que nunca investiu. Utilize as fontes fornecidas e apresente exemplos práticos.


### Prompt 2
Explique renda fixa para um estudante iniciante de tecnologia, organizando a resposta em:

- definição;

- funcionamento;

- exemplos;

- vantagens;

- riscos;

- cuidados.

### Prompt 3


Compare os principais investimentos de renda fixa (Tesouro Direto, CDB, LCI, LCA e Debêntures) utilizando as fontes fornecidas.
Organize a resposta em uma tabela contendo:

- O que é;

- Quem emite;

- Como funciona a rentabilidade;

- Nível de risco;

- Possui garantia do FGC ou não;

- Indicação para qual perfil de investidor.



